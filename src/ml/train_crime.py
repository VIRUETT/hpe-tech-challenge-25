...
def train_crime_model(csv_path: str, output_model_path: str = 'src/ml/crime_model.joblib'):
    ...
    if 'nombre_de_la_colonia' in df_processed.columns:
        for neighborhood in df_processed['nombre_de_la_colonia'].unique():
            ...
            crime_types = neighborhood_data['crime_type'].value_counts().head(3).to_dict() if 'crime_type' in neighborhood_data.columns else {}
            peak_hours = neighborhood_data['hour_int'].value_counts().head(3).index.tolist() if 'hour_int' in neighborhood_data.columns else []
            avg_density = neighborhood_data['índice_densidad_poblacional'].mean() if 'índice_densidad_poblacional' in neighborhood_data.columns else 0
            
            # Most common economic level
            economic_level = neighborhood_data['nivel_económico'].mode()[0] if ('nivel_económico' in neighborhood_data.columns and len(neighborhood_data) > 0) else 'unknown'
